...
namespace AutoLotConsoleApp
{
    class Program
    {
        ...
        private static void PrintAllInventory()
        {
            ...
            using (var context = new AutoLotEntities())
            {
                foreach (Car c in context.Cars) WriteLine(c);
                ...
                foreach (ShortCar c in context.Database.SqlQuery(typeof(ShortCar), "Select CarId,Make from Inventory")) WriteLine(c);

                foreach (Car c in context.Cars.Where(c => c.Make == "BMW")) WriteLine(c);

                WriteLine(context.Cars.Find(5));

                foreach (Car c in context.Cars) 
                    foreach (Orders o in c.Orders) WriteLine(o.OrderId);
                ...
                foreach (Orders o in context.Orders) context.Entry(o).Reference(x => x.Car).Load();
            }
        }                
        ...
        private static void RemoveRecord(int carId)
        {
            ...
            using (var context = new AutoLotEntities())
            {
                ...
                if (carToDelete != null)
                {
                    ...
                    if (context.Entry(carToDelete).State != EntityState.Deleted) throw new Exception("Unable to delete the record");
                    context.SaveChanges();
                }
            }
        }                
